# Capturing data lineages during program execution
...
1. **Generate a log of an execution**. A Program can send information about data instances to a log file during execution to document data-dependent operations. For example, the colnames() function in R lists the names of columns (i.e., variables) in a dataframe in order.  In Python, df.columns.tolist() will create a list of all VariableInstances in a DataframeInstance.  

1. **Metadata files**. The contents of a FileInstance can be described in a metadata file in a standard format, such as Data Documentation Initiative (DDI) and Ecological Metadata Language (EML).  Most data repositories use a standard metadata language to describe their holdings.

1. **Simplifying references to data objects**. When a languages allows multiple ways to reference a data object, these references should be resolved to a single SDTH DataInstance with a name (sdth:hasName) in SDTH. For example, a column in a dataframe in R (i.e., an sdth:VariableInstance) may be referenced by its index number or by its name. 
