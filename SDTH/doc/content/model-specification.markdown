...
An sdth:DataInstance is a collection of values created in a computer memory during the execution of an sdth:Program.  An sdth:DataInstance may be saved (sdth:savesFile) to a persistent medium as part of an sdth:FileInstance.  An sdth:DataInstance  retrieved (sdth:loadsFile) into computer memory from an sdth:FileInstance.

An sdth:DataInstance may be any kind of data collection, including arrays, matrices, lists, and datacubes.  SDTH pays special attention to instances of dataframes and variables, which are the data collections most often used in statistical analysis.  The sdth:DataframeInstance and sdth:VariableInstance described below are sub-classes of sdth:DataInstance.  SDTH also includes sdth:TextInstance and sdth:ImageInstance to describe two general kinds of outputs generated by programs.

When an sdth:ProgramStep creates a new sdth:DataInstance or changes any aspect of an existing sdth:DataInstance, a new URI is minted for the resulting sdth:DataInstance.  The new sdth:DataInstance should be linked to one or more sdth:DataInstances in the sdth:ProgramStep by a sdth:wasDerivedFrom, an sdth:elaboratedOf, or both.  This applies to all sub-classes of sdth:DataInstance as well.
...
An sdth:FileInstance is a data object stored on a persistent medium, such as a disk, tape, CD-ROM, or non-volatile memory storage device.  An sdth:FileInstance includes one or more sdth:DataInstances.

sdth:FileInstances may be in any format.  File formats used by statistical analysis programs to store dataframes usually include some metadata about variables, such as data type and label.  Simple formats, like comma-separated values (CSV), which contain very little metadata describing the meaning and attributes of values, may be accompanied by a separate metadata file. 

An sdth:FileInstance is created by an sdth:ProgramStep that executes an sdth:savesFile operation.  An sdth:savesFile transfers one or more sdth:DataInstances into the new sdth:FileInstance.

When an sdth:loadsFile is executed, the sdth:DataInstances in the sdth:FileInstance are restored in computer memory.
...
When a sdth:ProgramStep executes an sdth:loadsFile operation, the sdth:DataInstances in an external sdth:FileInstance are loaded into active computer memory.  A sdth:ProgramStep that executes a sdth:loadsFile command also performs an sdth:producesData operation.  

The sdth:DataframeInstance resulting from an sdth:loadsFile in an sdth:ProgramStep must get a new URI and be linked to an sdth:FileInstance with sdth:wasDerivedFrom.
...
When a sdth:ProgramStep executes an sdth:savesFile operation, one or more sdth:DataInstances are transferred to an external sdth:FileInstance.  An sdth:ProgramStep that executes a sdth:savesFile command also performs an sdth:consumesData operation.

The sdth:FileInstance resulting from an sdth:savesFile in an sdth:ProgramStep must get a new URI and be linked to one or more sdth:DataInstances with sdth:wasDerivedFrom.
